...
int ClientEdu::inregistreaza(const std::string& n,const std::string& p,const std::string& e,const std::string& parola,const std::string& rol){CerereEdu c;c.tip=TipCerereEdu::Inregistrare;c.campuri={{static_cast<std::uint16_t>(CampEdu::Nume),n},{static_cast<std::uint16_t>(CampEdu::Prenume),p},{static_cast<std::uint16_t>(CampEdu::Email),e},{static_cast<std::uint16_t>(CampEdu::Parola),parola},{static_cast<std::uint16_t>(CampEdu::Rol),rol}};const auto r=executaCerere(std::move(c));verificaSucces(r);const auto id=ProtocolEdu::cautaCamp(r.campuri,CampEdu::UtilizatorId);if(!id)throw ExceptieEdu("Raspunsul inregistrarii este incomplet.");return convertesteId(*id,"Id-ul utilizatorului");}

int ClientEdu::inregistreazaStudent(const std::string& n,const std::string& p,const std::string& e,const std::string& parola){return inregistreaza(n,p,e,parola,"student");}
...
namespace {
std::vector<RezultatEvaluarePublicEdu> decodificaListaRezultate(
    const RaspunsEdu& raspuns) {
    std::vector<RezultatEvaluarePublicEdu> rezultate;
    rezultate.reserve(raspuns.campuri.size());
    for (const auto& camp : raspuns.campuri) {
        if (camp.id != static_cast<std::uint16_t>(CampEdu::RezultatEvaluare)) {
            throw ExceptieEdu("Raspunsul listei de rezultate este invalid.");
        }
        rezultate.push_back(ProtocolEdu::decodificaRezultatEvaluare(camp.valoare));
    }
    return rezultate;
}
}

std::vector<RezultatEvaluarePublicEdu> ClientEdu::listeazaRezultateleMele() {
    CerereEdu cerere;
    cerere.tip = TipCerereEdu::ListeazaRezultateleMele;
    const auto raspuns = executaCerere(std::move(cerere));
    verificaSucces(raspuns);
    return decodificaListaRezultate(raspuns);
}

std::vector<RezultatEvaluarePublicEdu> ClientEdu::listeazaRezultateleEvaluarii(
    int evaluareId) {
    CerereEdu cerere;
    cerere.tip = TipCerereEdu::ListeazaRezultateleEvaluarii;
    cerere.campuri = {{static_cast<std::uint16_t>(CampEdu::EvaluareId),
