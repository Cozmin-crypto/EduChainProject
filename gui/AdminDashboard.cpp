...
AdminDashboard::AdminDashboard(std::shared_ptr<ApplicationContext> context,
                               QWidget* parent)
    : QWidget(parent), ui_(std::make_unique<Ui::AdminDashboard>()),
    ...
    connect(ui_->usersButton, &QPushButton::clicked, ui_->pagesStack,
            [this] { ui_->pagesStack->setCurrentIndex(1); });
    connect(ui_->coursesButton, &QPushButton::clicked, ui_->pagesStack,
            [this] { ui_->pagesStack->setCurrentIndex(2); });
    connect(ui_->statisticsButton, &QPushButton::clicked, ui_->pagesStack,
            [this] { ui_->pagesStack->setCurrentIndex(3); });
    connect(ui_->administrationButton, &QPushButton::clicked, ui_->pagesStack,
